...
This script is intended to be cleanly re-runable.
I should be able to run it top to bottom and recreate my database in place - this is for demo purposes.
...
-- the underlying SQL Server instance that is reading (running) this files
...
-- ===========================================================================================================
-- Section 1 - DDL
-- CREATE - Createing new tables, schemas, databases: NOT INDIVIDUAL ROWS
...
-- We have a schema already - MS SQL Server creates a default schema calle "dbo"
-- "Database Owner". If I create Author without specifying a schema, SQL Server
-- associates it with this default sbo schema. So the fill table name becomes:
-- dbo.Author
...
(
    ...
    -- I can optionally add some name constraints. If I don't name constraints,
    -- nothing breaks BUT I can make my life easier and make error messages more 
    -- functional/readable by explicitly naming my constaints
    CONSTRAINT PK_Author PRIMARY KEY (AuthorId),
    --If you THINK that you mightt need to alter a table's constraints,
    -- you should name them. It makes runnin ALTER TABLE commands easier later
...
(
    ...
    MemberId INT IDENTITY(1,1) NOT NULL PRIMARY KEY, --Fun fact, Identity will nor teuse any numbers even if deleted
    FirstName VARCHAR(50) NOT NULL,
    LastName VARCHAR(50) NOT NULL,
    Email VARCHAR(50) NOT NULL UNIQUE,
    -- Using a DEFAULT constraint, if no value is provided, the builr in GETDATE() function gets a value to satisfy the column
...
(
    ...
    -- CASCADE - VERY RISKY - If an author is deleted, all the their books go too
    -- SET NULL - Low risk - If an author is deleted, this AuthorID in Book is set to null (requires nullable colum)
    -- RESTRICT - SAFE - Default behavior, block deletion of an author if any books reference it
...
(
    ...
    -- Note: Technically, FK columns don't have to match the column in the thable they are a PK in.
...
-- We can set constraints for this ew column in line as well
...
-- I can use this ALTER TABLE + ALTER COLUMN to add or edit constraint
...
-- Single row insertion. It is best practice - borderline mandatory - to explicity list the columns
...
-- Getting back everything back from a table is fine, for our training. Usualliy we want to be
-- more specific
SELECT Title, PublishedYear
FROM dbo.Book
WHERE PublishedYear >= 2000; --Using WHERE as a filter.

-- I can use things like BETWEEN, LIKE, and IS combined my WHERE
...
-- I want title, categoryname from everybook who's category name is either software or testing
...
-- I want every book where title starts with "Clean"
SELECT Title 
FROM dbo.Book
WHERE Title 
LIKE '%Clean%';
...
-- Give me every book title where the category is software AND available copies is greate than 1
SELECT Title
FROM dbo.Book
...
-- is null, we don't use = . In SQL null doesn't equal anything. Its unknown, the absence of a value

-- LIKE vs IN vs =
-- = - matches one exact value
-- IN - matches any calue in the provided list
-- LIKE - matches some pattern with wildcards %

-- ORDER BY and DISTINCT
-- We probably want to be able to order the returned records base on some logic
-- atleast sometimes
SELECT Title, PublishedYear
FROM dbo.Book
ORDER BY PublishedYear DESC, Title ASC; --By default, SQL order by ASC (if ommited, its ASC)

--Using Distinct
-- Give me all the distinct category names that appear in dbo.Book
SELECT DISTINCT CategoryName
FROM dbo.Book
ORDER BY CategoryName; --ASC (default)
...
-- Give me the category name, and the count books in that category
-- where the count is more tan 2. Order the results by book count descending
SELECT CategoryName, COUNT(*) AS BookCount
FROM dbo.Book
GROUP BY CategoryName
HAVING COUNT(*) > 1 -- I cant use an alias name in HAVING, either a column that exists, or some function
...
-- WHERE runs before aby grouoing, and filters the raw rows that are then passed
...
-- COUNT() - 4 - COUNT(*) is different from COUNT(some_colum) - when you COUNT() a specific column, NULLS are ignored
-- SUM() - get the sum total soted in a column actoss many rows
-- AVG() - get the average value stored in a colummn across many rows - skip nulls
-- MIN(), MAX()
...
SELECT UPPER(LastName) AS LastUpper,
        ...
        DATEDIFF(DAY, JoinedDate, GETDATE()) --Takes 3 arguments, the datepart (year difference, day difference, etc)
FROM dbo.Member;

-- SQL Joins -- 
-- JOINs - JOINs are oe way to get information from multiple tables in the same query.
...
-- Lets do a JOIN across many to many
-- I want stuff from Authors and Books - I need to traverse BookAuthor

-- books with ALL their authors, through join table
...
-- FULL OUTER vs CROSS
-- FULL OUTER JOIN - Returns matched rows as well ther exist, as well as unmatched rows
SELECT b.Title, c.Name AS Category
FROM dbo.Book b
...
-- Subqueries
-- A Subquer y is a query inside another query
-- Use a join to combine columns from multiple tables into the output
-- Use a subquery to filter against a computer calue or set you DONT need in the output
...
-- Notice - we did te opposite earlier with a JOIN, whis could also
-- simply be a join
...
SELECT m.FirstName + ' ' + m.LastName Member,
    b.Title,
    c.Name as Category, 
    l.DueDate,
...
-- SQL despite not beign a programming language - has TRY and CATCH
...
BEGIN CATCH
    ...
    -- If we make it to the CATCH, something went wrong. Either a runtime error
...
END CATCH
    ...
    -- Every transactions ends in either a COMMIT or a ROLLBACK - you must provice logic for both
    ...
    -- set the isolation leve

    -- IN SQL Server there are four options.
    ...
    -- READ COMMITTED (Default) - Prevents dirty reads by requiring that data read by a statement must be
        --  committed befores it is processed.
    
    -- REPEATABLE READ - Places shared locks on all datra read by transaction and HOLDS THEM until the intire transaction
        -- No other transaction can modify or delete these rows during this time. Prevents some dirty reads,
        -- BUT other transactions can insert new rows that may bleed into your filtring for queries    
    -- SERIALIZABLE - Most restrictive. Places locks on rows that prevent other transactions from updating
        ...
        -- prevents ALL concurrency snomslird - and is also slow as hell. SQL Server has to create
        -- manage and delete a lot of locks. You can also create deadlocks.
    --
...
-- Views are like saved queries.
-- We can query the view as if it was a table
...
-- This is not a saved precompiled  datast- There is something called an indexed view
...
-- We have store procedures - a named program in the database. Parameters/arguments, logic
...
CREATE OR ALTER PROCEDURE dbo.usp_CheckoutBook
    ...
    @BookId INT,
    @MemberId INT,
    @Days INT = 14 -- how long is the loan for
AS
BEGIN
    SET XACT_ABORT ON;
        ...
        BEGIN TRANSACTION
            ...
            IF (SELECT AvailableCopies FROM dbo.Book WHERE BookId = @BookId) <= 0
                ...
                -- THROW takes 3 arguments
                    ...
                    -- message - some string describirn the error
                    -- state - an int between 0 and 255 describing where the error originated
                THROW 50000, 'No copies available ti check out.' ,1;
            INSERT INTO dbo.Loan (BookId, MemberId, DueDate)
    ...
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        THROW; -- Bubbling up the Try block error to wherever my stored procedures was called
    END CATCH
END;
...
-- Stored Procedure vs Uset Defined Function
-- A stored procedures DOES something. Writesc, transactions, returning result sets, etc. Called with EXEC
...
AS 
BEGIN
    -- Decalaring a variable in SQL that is scoped to this function
...
-- Indexes - a lookup structures like an index in a book
